local minhook = dofile("mods/evaisa.unshackle/lib/minhook.lua")("mods/evaisa.unshackle/bin")
minhook.initialize()

local SDL = dofile("mods/evaisa.unshackle/lib/sdl2_ffi.lua")
...
input.GetUIMousePos = function(self, gui)
	local game_config = dofile("mods/evaisa.unshackle/lib/game_config.lua")
    local input_manager = EntityGetWithName("mp_input_manager")
    if(input_manager == nil or not EntityGetIsAlive(input_manager))then
        input_manager = EntityLoad("mods/evaisa.unshackle/files/entities/input_manager.xml")
    end
